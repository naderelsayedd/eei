                            ...
                            <tbody>
                                ...
                                @foreach ($allCategories as $cat)
                                    <tr role="row">
                                        ...
                                        <td class="text-center">{{ $cat->created_at->format('Y-m-d H:i a') }}</td>
                                        <td class="text-center">{{ $cat->updated_at->format('Y-m-d H:i a') }}</td>
                                        <td class="text-center">
                                            <ul class="table-controls action-btns">
                                                <li>
                                                    <a href="{{ route('categories.edit', ['cat' => $cat->id]) }}"
                                                        title="update"
                                                ...
                                                <li>
                                                    <button type="button" title="delete"
                                                        ...
                                                        data-bs-target="#deletePartener{{ $partener->id }}">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="24"
                                                            height="24" viewBox="0 0 24 24" fill="none"
                                                            stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                                    ...
                                                    <div class="modal modal-lg fade fadeInDown"
                                                        id="deletePartener{{ $cat->id }}" tabindex="-1"
                                                            ...
                                                            <div class="modal-content">
                                                                ...
                                                                <div class="modal-body">
                                                                    <p class="modal-text">
                                                                        ...
                                                                        <br>
                                                                        <strong>Note: </strong> Partener will be deleted
                                                                        forever.
                                                                    </p>
                                                                </div>
                                                                <div class="modal-footer">
                                                                    <form
                                                                        action="{{ route('parteners.destroy', ['partener' => $partener->id]) }}"
                                                                        method="post">
                                                                        @method('delete')
                                                                        @csrf
...
@section('js')
    <script>
        ...
        c3.on('draw', function() {
            $('#partenersCount').text(c3.rows({
                search: 'applied'
            }).count());
        });


        $('.tableTitle').html("<span style='padding:6px;font-size:18px;'> Parteners (<span id='partenersCount'>" + c3.rows({
            search: 'applied'
        }).count() + "</span>) </span>");
        $('.table_btns').html(
