                    ...
                    <table id="zero-config" class="table dt-table-hover dataTable no-footer" style="width: 100%;" role="grid" aria-describedby="zero-config_info">
                        <thead>
                            <tr role="row">
                                ...
                                <th class="text-center sorting" tabindex="0" style="min-width: 80px;">Related To</th>
                                <th class="text-center sorting" tabindex="0" style="min-width: 80px;">Status</th>
                                <th class="text-center sorting" tabindex="0" style="min-width: 50px;">Order</th>
                        ...
                        <tbody>
                            ...
                            @foreach($sections as $section)
                                <tr role="row">
                                    ...
                                    <td class="text-center">{{ $section->page->name }}</td>
                                    <td class="text-center">
                                        <span class="badge @if($section->status == 1) badge-light-success @else badge-light-danger @endif">{{ $section->statusName }}</span>
                                    </td>
                                    <td class="text-center">{{ $section->order }}</td>
                                    <td class="text-center">{{ $section->created_at->format('Y-m-d H:i a') }}</td>
                                    <td class="text-center">{{ $section->updated_at->format('Y-m-d H:i a') }}</td>
                                    <td class="text-center">
                                        <ul class="table-controls action-btns">
                                            <li>
                                                <a href="{{ route('sections.edit', ['section' => $section->id]) }}" title="update" class="action-btn btn-update bs-tooltip mr-2 _effect--ripple waves-effect waves-light">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit-2"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg>
                                                </a>
                                            </li>
                                            <li>
                                                <button type="button" title="delete" class="action-btn btn-delete bs-tooltip mr-2 _effect--ripple waves-effect waves-light" data-bs-toggle="modal" data-bs-target="#deleteSection{{ $section->id }}">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-trash-2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg>
                                                </button>
                                                <!-- Delete Modal -->
                                                <div class="modal modal-lg fade fadeInDown" id="deleteSection{{ $section->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                        ...
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                <p class="modal-text">
                                                                    ...
                                                                    <br>
                                                                    <strong>Note: </strong> Section will be deleted forever.
                                                                </p>
                                                            </div>
...
@section('js')
    ...
    <script>
        ...
        c3.on( 'draw', function () {
            $('#sectionsCount').text(c3.rows( {search:'applied'} ).count());
        } );


        $('.tableTitle').html("<span style='padding:6px;font-size:18px;'> Pages Sections (<span id='sectionsCount'>"+c3.rows( {search:'applied'} ).count()+"</span>) </span>");
        $('.table_btns').html(
            '<a href="{{ route('sections.create') }}" class="btn btn-primary" tabindex="0"><span><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-plus"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg></span></a>'
        );
    </script>
